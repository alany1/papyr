...
Requires Node ≥ 20, the `claude` CLI on your PATH, and Xcode Command Line Tools
(to rebuild node-pty for Electron).
...
- The library location is stored in `~/Library/Application Support/papyr/config.json`;
  change it from the ⚙ button in the sidebar (the assistant restarts in the new folder).
